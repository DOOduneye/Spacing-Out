 ...
 * one instance of the enum to be created, and is used to connect to the database, and to execute
 * queries, etc.
 */
public enum Database {
  ...
  /**
   * Constructs the database and opens a connection to the database.
   */
  Database() {
...
//  }
  ...
  public void callAPI() throws SQLException {
    ...
    this.result = this.state.executeQuery("SELECT * FROM deck WHERE user_id = " + this.user.getKey());
  ...
  private Question[] getQuestions(String deckName) throws SQLException {
    this.result = this.state.executeQuery("SELECT *"
            + "FROM users u "
  ...
  private Answer[] getAnswers() throws SQLException {
    for (Map.Entry<Integer, Map.Entry<String, String>> question : questions.entrySet()) {
      this.result = this.state.executeQuery("SELECT * FROM answers WHERE question_id = "
              + question.getKey());
  ...
  private void setUser(int id) throws SQLException {
    this.userDecks = new ArrayList<>();
    this.result = this.state.executeQuery("SELECT * FROM users WHERE user_id = " + id);
